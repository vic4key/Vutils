  ...
  const ulong flags)
{
  ...
  ofn.Flags = flags | (action == action_type::save ? OFN_OVERWRITEPROMPT : 0);

  BOOL ret = action == action_type::open ? GetOpenFileNameA(&ofn) : GetSaveFileNameA(&ofn);
  ...
  const ulong flags)
{
  ...
  BOOL ret = action == action_type::open ? GetOpenFileNameW(&ofn) : GetSaveFileNameW(&ofn);
  bool result = ret == IDOK;
  if (result)
  {
    file_path.clear();
