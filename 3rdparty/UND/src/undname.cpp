 ...
 */
static char*    und_alloc(struct parsed_symbol* sym, unsigned int len)
{
    void*       ptr;
                           ...
                           struct array* a)
{
    ...
    if (!a->alloc)
    {
        _new = (char**)und_alloc(sym, (a->alloc = 32) * sizeof(a->elts[0]));
        if (!_new) return FALSE;
        a->elts = _new;
    }
    else if (a->max >= a->alloc)
    {
        _new = (char**)und_alloc(sym, (a->alloc * 2) * sizeof(a->elts[0]));
        if (!_new) return FALSE;
        memcpy(_new, a->elts, a->alloc * sizeof(a->elts[0]));
        a->alloc *= 2;
        a->elts = _new;
    }
