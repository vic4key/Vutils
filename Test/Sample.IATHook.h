...
DEF_SAMPLE(IATHook)
{
  VU_API_IAT_ATTACH(Test.exe, user32.dll, MessageBoxA);
  VU_API_IAT_ATTACH(Test.exe, user32.dll, MessageBoxW);

  MessageBoxA(vu::GetConsoleWindow(),  "The first message.",  "A", MB_OK);
  MessageBoxW(vu::GetConsoleWindow(), L"The first message.", L"W", MB_OK);

  VU_API_IAT_DETACH(Test.exe, user32.dll, MessageBoxA);
  VU_API_IAT_DETACH(Test.exe, user32.dll, MessageBoxW);
